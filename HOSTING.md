...
- **Keep the computer awake & powered.** Set power settings so the host doesn't
 sleep. On macOS, `setup-mac.sh` loads a `caffeinate -ims` agent that stops
 idle system sleep while the Mac is plugged in (the screen can still sleep
 normally) — keep it plugged in so it stays on.
